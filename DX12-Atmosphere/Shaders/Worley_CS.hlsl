RWTexture3D<float4> WorleyTexture : register(u0);
...
{
	float3 coord = float3(float(pixel.x) / 32.0, float(pixel.y) / 32.0, float(pixel.z) / 32.0);
...
{
	float4 worley = stackable3DNoise(DTid);
	WorleyTexture[DTid] = worley;
}