...
ByteArray ReadFileHelper(const wstring& fileName)
{
	...
	Utils::ByteArray byteArray = make_shared<vector<unsigned char> >(file.seekg(0, ios::end).tellg());
...
ByteArray Inflate(ByteArray CompressedSource, int& err, uint32_t ChunkSize = 0x100000)
{
	...
	Utils::ByteArray byteArray = make_shared<vector<unsigned char> >(strm.total_out);
...
ByteArray DecompressZippedFile(wstring& fileName)
{
	...
	{
		std::cout << L"Couldn't unzip file " << fileName.c_str() << L":  Error = " << error << std::endl;
		return NullFile;
	}
...
ByteArray Utils::ReadFileSync(const wstring& fileName)
{
	return ReadFileHelperEx(make_shared<wstring>(fileName));
}

task<ByteArray> Utils::ReadFileAsync(const wstring& fileName)
{
